...
static int ce147_get_main_sw_fw_version(struct v4l2_subdev *sd)
{
	...
	if ((MAIN_SW_DATE_INFO[0] == 0x00) && (MAIN_SW_DATE_INFO[1] == 0x00)
		...
		{
			err = ce147_read_fw_bin("/system/vendor/firmware/CE147F02_seh.bin", fw_data, &fw_size);
			printk("Inside ce147_get_main_sw_fw_version :  seh bin folder!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		}
		else //Default SEM if(state->fw.major==0x05)                      //For SEM version
		{
			err = ce147_read_fw_bin("/system/vendor/firmware/CE147F02_sem.bin", fw_data, &fw_size);
...
static int ce147_set_preview_start(struct v4l2_subdev *sd)
{
	...
	printk("%s: width: %d, height: %d, fps: %d\n", __func__, state->pix.width, state->pix.height, state->fps);
...
static void ce147_init_parameters(struct v4l2_subdev *sd)
{
	...
	state->ev = 0;
	state->wb = WHITE_BALANCE_AUTO;
	state->effect =	IMAGE_EFFECT_NONE;
	state->iso = ISO_AUTO;
...
static int ce147_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	* which can be used in image capture also */
	...
	default:
		dev_err(&client->dev, "%s: no such control: %#x (%d)\n", __func__, ctrl->id, (ctrl->id - offset));
		break;
	}
				...
				struct v4l2_ext_control *ctrl)
{
	...
	case V4L2_CID_CAMERA_GPS_LATITUDE:
		tempGPSType = (struct gps_info_common *)ctrl->string;
	...
	case V4L2_CID_CAMERA_GPS_TIMESTAMP:
		...
		//temp = *((unsigned long	*)ctrl->string);
		if (ctrl->string)
			state->gpsInfo.gps_timeStamp = *((unsigned long *)ctrl->string);
		else
			err = -EINVAL;
		break;

	case V4L2_CID_CAMERA_EXIF_TIME_INFO:
		state->exifTimeInfo = (struct tm *)ctrl->string;
		err = 0;
		break;

	case V4L2_CID_CAMERA_GPS_PROCESSINGMETHOD:
		//temp2 = ((char *)ctrl->string);
		if (ctrl->string)
			strcpy(state->gpsInfo.gps_processingmethod, ctrl->string);
		else
			err = -EINVAL;
		break;
	default:
