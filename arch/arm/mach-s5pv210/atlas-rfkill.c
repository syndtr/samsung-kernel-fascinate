...
static int __init atlas_rfkill_probe(struct platform_device *pdev)
{
	int irq;
	int ret;
...
static struct platform_driver atlas_device_rfkill = {
	.probe = atlas_rfkill_probe,
	.driver = {
		.name = "bt_rfkill",
		.owner = THIS_MODULE,
	},
...
static int __init atlas_rfkill_init(void)
{
	int rc = 0;
	rc = platform_driver_register(&atlas_device_rfkill);

	return rc;
}

module_init(atlas_rfkill_init);
MODULE_DESCRIPTION("atlas rfkill");
MODULE_LICENSE("GPL");
