/* arch/arm/mach-s5pv210/atlas-gpio.c
 *
...
static struct gpio_init_data atlas_init_gpios[] = {
...
static void s3c_config_gpio_table(void)
{
	u32 i, gpio;

	for (i = 0; i < ARRAY_SIZE(atlas_init_gpios); i++) {
		gpio = atlas_init_gpios[i].num;
		if (system_rev <= 0x07 && gpio == S5PV210_GPJ3(3)) {
			s3c_gpio_cfgpin(gpio, S3C_GPIO_OUTPUT);
			gpio_set_value(gpio, S3C_GPIO_SETPIN_ONE);
		} else if (gpio <= S5PV210_MP05(7)) {
			s3c_gpio_cfgpin(gpio, atlas_init_gpios[i].cfg);
			s3c_gpio_setpull(gpio, atlas_init_gpios[i].pud);

			if (atlas_init_gpios[i].val != S3C_GPIO_SETPIN_NONE)
				gpio_set_value(gpio, atlas_init_gpios[i].val);

			s3c_gpio_set_drvstrength(gpio, atlas_init_gpios[i].drv);
		}
	}
}

/* this table only for B4 board */
static unsigned int atlas_sleep_gpio_table[][3] = {
...
static void s3c_config_sleep_gpio_table(void)
{
	u32 i, gpio;

	for (i = 0; i < ARRAY_SIZE(atlas_sleep_gpio_table); i++) {
		gpio = atlas_sleep_gpio_table[i][0];
		s3c_gpio_slp_cfgpin(gpio, atlas_sleep_gpio_table[i][1]);
		s3c_gpio_slp_setpull_updown(gpio, atlas_sleep_gpio_table[i][2]);
	}
}
