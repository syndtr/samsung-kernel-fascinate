/* arch/arm/mach-s5pv210/atlas-vibrator.c
 *
...
static void atlas_vibrator_off(void)
{
...
static int atlas_vibrator_get_time(struct timed_output_dev *dev)
{
...
static void atlas_vibrator_enable(struct timed_output_dev *dev, int value)
{
...
static struct timed_output_dev to_dev = {
	.name		= "vibrator",
	.get_time	= atlas_vibrator_get_time,
	.enable		= atlas_vibrator_enable,
};

static enum hrtimer_restart atlas_vibrator_timer_func(struct hrtimer *timer)
{
	schedule_work(&vibdata.work);
	return HRTIMER_NORESTART;
}

static void atlas_vibrator_work(struct work_struct *work)
{
	atlas_vibrator_off();
}

static int __init atlas_init_vibrator(void)
{
	int ret = 0;

	hrtimer_init(&vibdata.timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	vibdata.timer.function = atlas_vibrator_timer_func;
	INIT_WORK(&vibdata.work, atlas_vibrator_work);
...
device_initcall(atlas_init_vibrator);
